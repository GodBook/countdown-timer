...
公开仓库：[GodBook/countdown-timer](https://github.com/GodBook/countdown-timer)。从 [最新版本](https://github.com/GodBook/countdown-timer/releases/latest) 下载 APK，或使用本地 `dist/countdown-timer-1.1.3.apk`。支持 Android 8.0（API 26）及以上，目标平台为 Android 16（API 36）。将 APK 发送到手机，通过文件管理器打开；如系统要求，允许该文件管理器安装应用。

首次打开允许通知。在“后台计时保护”中点击“允许后台持续计时”，按系统提示允许电池优化豁免；iQOO/vivo 还需在手机系统设置中允许自启动和后台运行／后台高耗电（名称随系统版本变化）。只开启悬浮窗不能保证后台按时响铃。Android 12 还需按应用提示允许“闹钟和提醒”。输入时、分、秒，或选择 1／5／10／25 分钟，选择提醒方式，点击“开始计时”。下滑通知栏可查看剩余时间，或暂停、继续、取消。
...
- 后台视觉弹窗保持到确认关闭，系统显示“结束提醒待确认”的服务通知；点击弹窗或通知的完成按钮关闭。返回应用会移除悬浮窗，转为应用内弹窗。响铃超时不会自动关闭视觉提醒。
- 1.1.3 在三种提醒模式开始时均准备独立前台计时服务，不再依赖悬浮窗权限；精确闹钟直接启动服务。运行时持有有超时保护的 CPU 唤醒锁，并等待单调时钟截止时间；暂停、重置、结束及服务销毁会释放，可能增加计时期间的耗电。系统强制停止仍会阻断提醒。
- 授予悬浮窗权限后，可在待开始状态点击“5 秒后台弹窗测试（返回桌面）”，启动一次纯视觉测试计时并返回桌面。iQOO/vivo 如仍被拦截，可从应用提供的设置入口检查后台弹出界面、自启动和后台运行选项（以手机实际提供的选项为准）。权限已授予与服务实际运行分开显示。
...
当前应用 ID 为 `com.awxds.countdowntimer`，版本 `1.1.3`，`versionCode=5`。发行 APK 使用专用 RSA 签名，调试签名与其不同。后续更新必须保留应用 ID、使用相同发行密钥，并提高 `app/build.gradle.kts` 中的 `versionCode`。
...
若出现回到应用才提醒，先确认上述后台权限，再点击“提醒诊断”复制记录。日志会注明结束来自后台服务、系统闹钟还是前台补处理；日志只保存在设备本地，不自动上传。`verification/BACKGROUND-1.1.3.md` 记录本版验证和未覆盖事项。
...
具体执行结果、环境和未验证项见 `verification/RESULTS.md`；1.1.2 的正式 APK 桌面弹窗、其他应用上方弹窗、进程终止恢复及覆盖安装记录见 `verification/POPUP-1.1.2.md`。Android 8～15 兼容性通过 API 级别分支及静态检查覆盖，尚不等同于这些版本逐一真机验收。
